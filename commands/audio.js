...
let category = {
	commands: {},
	description: "Voice channel stuff.",
	name: "audio",
	printName: ":speaker: Audio"
}
 ...
 * TODO: Play chatsounds one after another, try using the same folder for them (prioritize long chatsounds over short ones)
 ...
 * TODO: Make sure # numbers stay the same over "sessions"
...
category.commands.play = {
	callback: async function(msg, line) {
		if (!client.soundListKeys) { msg.reply("sound list hasn't loaded yet."); return }

		let vc = await client.getCommands().join.callback(msg)
...
category.commands.volume = {
	callback: function(msg, line, vol) {
		let vc = msg.guild.me.voiceChannel
...
category.commands.search = {
	callback: function(msg, line, ...options) {
		...
		let results = []
		for (key in client.soundListKeys) {
			if (client.soundListKeys.hasOwnProperty(key)) {
